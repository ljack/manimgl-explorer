(function() {
  ...
  files["_2025/zeta/play.py"] = {
    description: "Riemann zeta function playground: interactive visualizations of zeta(s) in the complex plane, analytic continuation, and the critical strip.",
...
class ZetaLogDerivSum(ZetaSum):
    ...
    annotations: {
      ...
      21: "ZetaSum extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      28: "setup() runs before construct(). Used to initialize shared state and add persistent mobjects.",
      30: "ValueTracker holds a numeric value that can be animated. Other mobjects read it via get_value() in updaters.",
      38: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      48: "The animate builder pattern: creates an animation interpolating from the current state to the state after the chained method call.",
      ...
      55: "ComplexPlane extends NumberPlane for complex number visualization. Points map to complex numbers directly.",
      56: "ComplexPlane extends NumberPlane for complex number visualization. Points map to complex numbers directly.",
      70: "GlowDot is a radial gradient dot with a soft glow effect, rendered via a special shader.",
      72: "State-based updater: called every frame with the mobject. Used for reactive positioning and following other objects.",
      73: "Tex renders LaTeX mathematical expressions. Use raw strings (r\"...\") to avoid backslash escaping issues.",
      ...
      96: "State-based updater: called every frame with the mobject. Used for reactive positioning and following other objects.",
      113: "Exponential function: fundamental to Laplace transforms, signal processing, and growth/decay models.",
      147: "Class ZetaLogDerivSum inherits from ZetaSum.",
      153: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      155: "Reorients the 3D camera: (theta, phi, gamma, center, height) sets horizontal rotation, elevation, roll, look-at point, and zoom level.",
      ...
      169: "The animate builder pattern: creates an animation interpolating from the current state to the state after the chained method call.",
      ...
      171: "The animate builder pattern: creates an animation interpolating from the current state to the state after the chained method call.",
      173: "The animate builder pattern: creates an animation interpolating from the current state to the state after the chained method call.",
      189: "Exponential function: fundamental to Laplace transforms, signal processing, and growth/decay models.",
    }
  };
