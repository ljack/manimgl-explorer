(function() {
  ...
  files["_2024/inscribed_rect/helpers.py"] = {
    description: "Helper functions for the inscribed rectangle problem: parametric curve generators, Möbius strip constructions, and geometric utility functions.",
...
def sudanese_band_func(u, v):
    ...
    annotations: {
      ...
      34: "GlowDot is a radial gradient dot with a soft glow effect, rendered via a special shader.",
      45: "Linearly interpolates between two values: result = a + alpha*(b-a), where alpha ranges 0 to 1.",
      49: "np.linspace creates evenly spaced values over an interval — essential for parametric sampling.",
      62: "Returns the Euclidean length of a vector. ManimGL utility wrapping np.linalg.norm.",
      92: "np.linspace creates evenly spaced values over an interval — essential for parametric sampling.",
      107: "Computes the angle between two vectors using the dot product formula: cos(θ) = (a·b)/(|a||b|).",
      151: "Rotates a 2D vector by the given angle. Equivalent to multiplying by a rotation matrix.",
      152: "Rotates a 2D vector by the given angle. Equivalent to multiplying by a rotation matrix.",
      153: "Trigonometric functions: used for circular/spherical geometry, wave physics, and periodic motion.",
      168: "Returns the Euclidean length of a vector. ManimGL utility wrapping np.linalg.norm.",
      177: "Exponential function: fundamental to Laplace transforms, signal processing, and growth/decay models.",
      178: "Exponential function: fundamental to Laplace transforms, signal processing, and growth/decay models.",
      180: "Rotates a 2D vector by the given angle. Equivalent to multiplying by a rotation matrix.",
      182: "Rotates a 2D vector by the given angle. Equivalent to multiplying by a rotation matrix.",
      183: "Rotates a 2D vector by the given angle. Equivalent to multiplying by a rotation matrix.",
    }
  };

  files["_2024/inscribed_rect/loops.py"] = {
    description: "Main scenes for the inscribed rectangle theorem proof. Visualizes how pairs of points on a closed curve map to a Möbius strip, and why an inscribed rectangle must exist.",
    code: `from __future__ import annotations
...
class MusicalIntervalsAsPairs(InteractiveScene):
    ...
    annotations: {
      1: "Enables PEP 604 union types (X | Y) and postponed evaluation of annotations for cleaner type hints.",
      3: "Standard ManimGL wildcard import: provides Scene, Mobject hierarchy, animations, constants (PI, TAU, UP, RIGHT, etc.), numpy as np, and utility functions.",
      5: "Imports * from the _2024.inscribed_rect.helpers module within the 3b1b videos codebase.",
      14: "LoopScene extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      77: "State-based updater: called every frame with the mobject. Used for reactive positioning and following other objects.",
      85: "Tex renders LaTeX mathematical expressions. Use raw strings (r\"...\") to avoid backslash escaping issues.",
      93: "Class StateThePuzzle inherits from LoopScene.",
      ...
      102: "self.play() executes one or more animations simultaneously and waits for them to complete.",
      103: "ShowCreation draws a VMobject's stroke progressively from start to end.",
      104: "Write animates text/LaTeX appearing stroke by stroke, simulating handwriting.",
      109: "Subdivides existing bezier curves to increase point density. Needed before applying nonlinear transformations for smooth results.",
      111: "ValueTracker holds a numeric value that can be animated. Other mobjects read it via get_value() in updaters.",
      112: "Creates a list of colors smoothly interpolated between the given endpoints.",
      ...
      123: "ShowCreation draws a VMobject's stroke progressively from start to end.",
      124: "self.play() executes one or more animations simultaneously and waits for them to complete.",
      125: "Write animates text/LaTeX appearing stroke by stroke, simulating handwriting.",
      126: "FadeOut transitions a mobject from opaque to transparent.",
      128: "self.wait() pauses for 1 second (default), allowing updaters and ambient animations to run.",
      144: "Transform smoothly morphs one mobject into another by interpolating their points.",
      ...
      152: "Text renders plain text with the default font. Supports substring indexing for partial styling.",
      158: "self.play() executes one or more animations simultaneously and waits for them to complete.",
      159: "FadeIn transitions a mobject from transparent to opaque, optionally with a directional shift.",
      160: "FadeOut transitions a mobject from opaque to transparent.",
      162: "self.wait() pauses for 1 second (default), allowing updaters and ambient animations to run.",
      180: "Tex renders LaTeX mathematical expressions. Use raw strings (r\"...\") to avoid backslash escaping issues.",
      181: "Tex renders LaTeX mathematical expressions. Use raw strings (r\"...\") to avoid backslash escaping issues.",
      186: "Subdivides existing bezier curves to increase point density. Needed before applying nonlinear transformations for smooth results.",
      195: "self.play() executes one or more animations simultaneously and waits for them to complete.",
      196: "Transform smoothly morphs one mobject into another by interpolating their points.",
      206: "self.wait(n) pauses the scene for n seconds, allowing updaters and animations to run.",
      211: "Text renders plain text with the default font. Supports substring indexing for partial styling.",
      218: "self.play() executes one or more animations simultaneously and waits for them to complete.",
      219: "FadeOut transitions a mobject from opaque to transparent.",
      220: "ShowCreation draws a VMobject's stroke progressively from start to end.",
      222: "self.play() executes one or more animations simultaneously and waits for them to complete.",
      223: "The animate builder pattern: creates an animation interpolating from the current state to the state after the chained method call.",
      258: "Class ReframeToPairsOfPoints inherits from LoopScene.",
      259: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      423: "Class ShowTheSurface inherits from LoopScene.",
      424: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      880: "Class ChangeTheSurface inherits from ShowTheSurface.",
      881: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      996: "ParameterizeTheLoop extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      997: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      1724: "Class DiscussOrderOfPoints inherits from LoopScene.",
      1725: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      1838: "Class MapTheStripOntoTheSurface inherits from ShowTheSurface.",
      1839: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      2053: "Class AmbientRectangleSearch inherits from ShowTheSurface.",
      2054: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      2087: "Class GenericLoopPair inherits from ShowTheSurface.",
      2088: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      2110: "SudaneseBand extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      2111: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      2157: "Class ShowSurfaceReflection inherits from ShowTheSurface.",
      2158: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      2219: "ConstructKleinBottle extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      2220: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      2475: "Class PuzzleOverMobiusDiagram inherits from ConstructKleinBottle.",
      2476: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      2525: "Class ShowAngleInformation inherits from ShowTheSurface.",
      2526: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      2686: "Class RectanglesOfAllAspectRatios inherits from LoopScene.",
      2687: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      2803: "Class AllAspectRatioXMaxTree inherits from RectanglesOfAllAspectRatios.",
      2842: "Class AllAspectRatioPi inherits from RectanglesOfAllAspectRatios.",
      2847: "Class TrackTheAngle inherits from ShowTheSurface.",
      2852: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      2953: "Class TrackTheAngleForFractal inherits from TrackTheAngle.",
      2984: "Class KochZoom inherits from TrackTheAngleForFractal.",
      2985: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      2995: "Class MobiusStripsAndKleinBottlesIn4D inherits from ConstructKleinBottle.",
      2996: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      3035: "MusicalIntervalsAsPairs extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      3036: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
    }
  };

  files["_2024/inscribed_rect/supplements.py"] = {
    description: "Supplementary scenes exploring edge cases, alternative proofs, and deeper topology of the inscribed rectangle problem.",
...
class EndScreen(PatreonEndScreen):
    title_text = ""`,
    annotations: {
      5: "Class UnsolvedQuestion inherits from TeacherStudentsScene.",
      6: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      16: "Class AskWhoCares inherits from TeacherStudentsScene.",
      17: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      44: "PurePuzzleToApplication extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      45: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      92: "WhatIsTopology extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      93: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      170: "ThreeShapes extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      171: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      192: "ThisIsTheSame extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      193: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      202: "LabelMapping extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      203: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      241: "Class ComplainAboutQuestion inherits from TeacherStudentsScene.",
      242: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      256: "Class RememberThat inherits from TeacherStudentsScene.",
      257: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      265: "Class AskAboutACircle inherits from TeacherStudentsScene.",
      266: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      278: "AskAboutProvingCollision extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      279: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      289: "GraphOfBellCurve extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      290: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      307: "GraphLabel extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      308: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      319: "ChekhovsGun extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      320: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      350: "XXLabel extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      351: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      358: "CommutativeDiagram extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      361: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      398: "ContinuousAssociation extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      399: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      415: "Class ThinkLikeATopologist inherits from TeacherStudentsScene.",
      416: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      424: "RandyPuzzling extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      425: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      440: "UnorderedPair extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      441: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      447: "XYtoYX extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      448: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      454: "XXOnEdge extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      455: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      461: "StripClaim extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      462: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      556: "MobiusStripTextReflection extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      557: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      570: "AskAboutUnsolved extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      571: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      579: "Class AskAbout4DEmbeddings inherits from TeacherStudentsScene.",
      580: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      627: "GreeneAndLobb extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      628: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      650: "GreeneLobbTheorem extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      651: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      677: "SmoothImplication extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      678: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      697: "ProblemSolvingToRecreation extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      698: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      748: "WriteTopologicalSpace extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      749: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      757: "Class AskAboutTopology inherits from TeacherStudentsScene.",
      758: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      780: "PlaylistMention extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      781: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      811: "AskWhat extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      812: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      831: "Class EndScreen inherits from PatreonEndScreen.",
    }
  };
