(function() {
  ...
  files["_2024/antp/main.py"] = {
    description: "Animation scenes for an 'ask a new teacher personal' advice-style video segment.",
...
class WannaProve(InteractiveScene):
    ...
    annotations: {
      ...
      3: "Imports ChartBars from the _2023.clt.main module within the 3b1b videos codebase.",
      6: "TwinPrimScrolling extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      7: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      32: "Tex renders LaTeX mathematical expressions. Use raw strings (r\"...\") to avoid backslash escaping issues.",
      56: "Linearly interpolates between two values: result = a + alpha*(b-a), where alpha ranges 0 to 1.",
      62: "Interpolates between colors in HSL space for perceptually uniform gradients.",
      85: "Timeline extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      ...
      126: "Returns the Euclidean length of a vector. ManimGL utility wrapping np.linalg.norm.",
      134: "InfinitePrimes extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      135: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      145: "Linearly interpolates between two values: result = a + alpha*(b-a), where alpha ranges 0 to 1.",
      156: "Pins this mobject to the camera frame (HUD-style) so it stays fixed when the 3D camera moves.",
      158: "Reorients the 3D camera: (theta, phi, gamma, center, height) sets horizontal rotation, elevation, roll, look-at point, and zoom level.",
      ...
      164: "FadeIn transitions a mobject from transparent to opaque, optionally with a directional shift.",
      193: "ThoughtBubble extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      194: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      201: "SimpleBubble extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      202: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      206: "EuclidProof extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      ...
      212: "Text renders plain text with the default font. Supports substring indexing for partial styling.",
      220: "Arrow creates a line with an arrowhead. path_arc parameter curves the arrow along a circular arc.",
      233: "Tex renders LaTeX mathematical expressions. Use raw strings (r\"...\") to avoid backslash escaping issues.",
      236: "Tex renders LaTeX mathematical expressions. Use raw strings (r\"...\") to avoid backslash escaping issues.",
      243: "self.play() executes one or more animations simultaneously and waits for them to complete.",
      254: "Write animates text/LaTeX appearing stroke by stroke, simulating handwriting.",
      255: "self.wait() pauses for 1 second (default), allowing updaters and ambient animations to run.",
      322: "PrimeDensityHistogram extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      323: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      370: "PrimesNearMillion extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      371: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      478: "PrimePanning extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      479: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      506: "SieveWithMod extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      507: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      664: "DensityFormula extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      665: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      730: "OldGapsInPrimes extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      731: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      735: "NewGapsInPrimes extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      736: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      806: "CrankEmail extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      807: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      1012: "SieveOfEratosthenes extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      1017: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
      1066: "Class GiantSieve inherits from SieveOfEratosthenes.",
      1079: "WannaProve extends InteractiveScene. InteractiveScene provides self.frame (3D camera), mouse/keyboard interaction, and the full OpenGL rendering pipeline.",
      1080: "construct() is the main entry point for this scene. ManimGL calls it when the scene runs — all animation logic goes here.",
    }
  };
